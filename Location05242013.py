...
if file_case == 1:
    ...
    for file in fasta_database:
        ...
        del fasta
######################################################################
print 'Combine same peptide sequence'
'''
peptides = {}
for line in spectrum_file:
    data = line.strip()
    data = data.split('\t')
    current_pep = CleanPeptideString(data[8])
    if peptides.has_key(current_pep):
        peptides[current_pep].append(data)
    else:
        peptides[current_pep] = [[data]]

### temp loading datas::: needed to be replaced with cPickle from suwoo here.
pepdic = {}
pep_list = peptides.keys()
for pep in pep_list:
    pepdic[pep] = [[],{},[],0,0.98,0]  # [[spectra file information],[location information],[extra needed],peptides count,MS/MS FDR,location count] 
#location information saved in pepdic[pep][1]
#{location information} = [{chr1:[[location1],[location2],[location3]],chr2:[[location1],[location2]]}]
'''
...
####################################################################### grouping
print 'Grouping peptides events'
