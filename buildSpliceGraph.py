...
def    formatChrNameForHuman(reference_name_str):
	if reference_name_str == "1":
		...
		return "Y"
	elif reference_name_str == "MT":
		return "MT"
	else:
		return -1


def    formatChrNameForHuman_old(reference_name_str):
	if reference_name_str == "1":
		return "chr1"
	elif reference_name_str == "2":
		return "chr2"
...
for i in range(0,len(sys.argv)/2):
	...
	elif sys.argv[i*2+1] == "-sp":
		SpliceDB_tools_directory = sys.argv[i*2+2] #-o
	else:
...
if BAM_SAM_TMP_file_directory == "":
	...
	if os.path.isdir(Output_TMP_file_name):
		Output_TMP_file_name = os.path.join(Output_TMP_file_name,"Merged_BAM_SAM_SPL.spl")
	elif Output_TMP_file_name.endswith(".spl") == False:
		print "splice .spl file name should end with .spl"
		exit()
if Output_TMP_file_name == "":
	print "Output spl file name not specified"
	exit()
...
for F in Files:
	...
	if F.endswith(".fa") or F.endswith(".fasta"):
		if F.startswith("chr") or F.startswith("Chr") or F.startswith("CHR"):
			chr_ref_tmp = F.split("_")[0].replace("chr","")
			...
			if os.path.exists(Stub + "_0.fasta") == False:
				...
				if SpliceDB_tools_directory == "":
					cmd_line_str = "java -jar SixFrameBuilder.jar -r " + file_path_and_name + " -w " + Sixframe_file_name_per_chr[chr_ref_tmp]
				else:
					cmd_line_str = "java -jar " + SpliceDB_tools_directory + "/SixFrameBuilder.jar -r " + file_path_and_name + " -w " + Sixframe_file_name_per_chr[chr_ref_tmp]
				#os.system(cmd_line_str)
		else:
			SourceFile = open(file_path_and_name,'r')
			data = SourceFile.readline()
			data = data.strip()
			data = data.replace(">","")
			data = data.split(" ")[0]
			...
			if data.startswith("chr"):
				...
				print "dna file", F, "starts with wrong chr name", data
...
for F in Files:
	...
	if F.endswith(".spl") or F.endswith(".tmp"):
		file_path_and_name = os.path.join(BAM_SAM_TMP_file_directory,F)
		print " -Processing .spl file :",F
		SourceFile = open(file_path_and_name,'r')
		ID = 0
...
for chromosome in temp:	
	...
	if SpliceDB_tools_directory == "":
		...
		cmd_line_str = "java -Xmx" + str(java_mem_in_MB) + "M -jar " + SpliceDB_tools_directory + "/ConstructSpliceGraphFromTmp.jar"
		cmd_line_str = cmd_line_str + " -p " + Output_TMP_file_name_per_chr
		cmd_line_str = cmd_line_str + " -w " + Output_ms2db_file_name_per_chr
		cmd_line_str = cmd_line_str + " -s " + dna_TRIE_file_name_per_chr[chromosome]
	...
	if SpliceDB_tools_directory == "":
		...
		cmd_line_str = "python " + SpliceDB_tools_directory + "/ACGT03102013_split.py " + Output_ms2db_file_name_per_chr + " " + Output_fasta_file_name_per_chr + " 30"
